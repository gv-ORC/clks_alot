...
#include <Vrecovery_tb.h>

class Recovery : public WrappedDevice<Vrecovery_tb>
{
public:
    Recovery(const int argc, const char **argv)
        : Recovery(argc, argv, 0) {}

    Recovery(const int argc, const char **argv, const int seed)
        : WrappedDevice<Vrecovery_tb>(argc, argv, seed)
    {
        device_->clk = 1;
        device_->clk_en = 0;
        device_->sync_rst = 0;
